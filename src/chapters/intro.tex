...
What I tried to do was to replace the Wi-Fi Direct interface with an interface for a secondary Wi-Fi 802.11 connection then implement my own power-save algorithm but this proved to be a show-stopper because the firmware for Nexus 5 is closed-source and it was impossible to make the stack work in parallel with two virtual interfaces in managed mode.
...
The average power for 120s when the phone is connected just to the AP but no data is transferred is 60.57mW. This value should be lower but it seems that some IPV6 Router Advertisements messages introduce some spikes in the power consumption graph from \labelindexref{Figure}{img:ap-only}.
...
In \labelindexref{Figure}{img:notraffic}, "Single Interface PS algo" represents the standard IEEE802.11 algorithm implementation from the ath9k_htc driver, while the 'No PS algorithm' measures the average power when power save is disabled.
...
\captionof{figure}{Comparison between Power Save Algorithms}
\label{img:notraffic}
\end{figure}
...
\labelindexref{Figure}{img:cdfdouble} shows the RTT CDF for ping packets generated at different intervals, in case of the double wake-up power save algorithm. The CDF shows that as the ping interval is increased the RTT for ping packets decreases because the client stays awake more time.

\begin{figure}
\includegraphics{src/img/cdfdouble.eps}
\captionof{figure}{Double wake-up PS algo CDF for different ping intervals}
\label{img:cdfdouble}
\end{figure}
